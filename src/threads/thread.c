...
static void thread_recalc_priority(struct thread *t, void *aux UNUSED);
static bool intr_thread_check_priority(void);
...
thread_init (void) 
{
    ...
    initial_thread->nice = 0; // A thread inicial (main) começa com nice 0
}
...
/* Creates a new kernel thread named NAME with the given initial
   PRIORITY, which executes FUNCTION passing AUX as the argument, and adds it to the ready queue. Returns the thread identifier for the new thread, or TID_ERROR if creation fails.

   If thread_start() has been called, then the new thread may be
   scheduled before thread_create() returns.  It could even exit
   before thread_create() returns.  Contrariwise, the original
   thread may run for any amount of time before the new thread is scheduled. 
